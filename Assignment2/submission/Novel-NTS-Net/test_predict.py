...
for i, data in enumerate(testloader):
    with torch.no_grad():
        ...
        _, concat_logits, _, _, _ = net(x = img, imclass=imclass)
        # calculate loss
...
for index, label in enumerate(out):
    if (label[0] == 'a'):
        f3.write(img_txt_file[index] + " " + "aircrafts aircrafts@" + label[1:] + '\n')
    if (label[0] == 'b'):
        f3.write(img_txt_file[index] + " " + "birds birds@" + label[1:] + '\n')
    if (label[0] == 'c'):
        f3.write(img_txt_file[index] + " " + "cars cars@" + label[1:] + '\n')
    if (label[0] == 'd'):
        f3.write(img_txt_file[index] + " " + "dogs dogs@" + label[1:] + '\n')
    if (label[0] == 'f'):
        f3.write(img_txt_file[index] + " " + "flowers flowers@" + label[1:] + '\n')
