...
class Predict_data():
    def __init__(self, root, data_len=None):
        self.root = root
...
class PredictClass():
    def __init__(self, imclass, label=0, data_len=None):
        self.imclass = imclass
        # img_txt_file = open(os.path.join('coarse_output', imclass + '.txt'), 'r').readlines()
        ...
        self.test_img = test_file_list
        self.test_img = [scipy.misc.imread(os.path.join('test_images', test_file)) for test_file in test_file_list]
        # self.test_label = label_list
        self.test_label = [x for x in (label_list)][:data_len]
